...
void get_kas(){
  ...
  double ax = accelermetor.x();
  double ay = accelermetor.y();
  double az = accelermetor.z();
  Serial.println(ax);
  Serial.println(ay);
  Serial.println(az);
  kas_data[0]=ax;
...
void get_acc2() {
  ...
  acc_data[3] = abs(acc_data[0]) + abs(acc_data[1]) + abs(acc_data[2]);
}
  ...
  //Serial.println("made");
    while (Serial2.available() > 0) {
        ...
        if (gps.location.isUpdated()) {
           ...
           // ゴール地点の緯度,経度
            double goal_lat = 40.1422826459;// ゴールの緯度
            double goal_lon = 139.9873883750; // ゴールの経度
...
void pid() {
  while (true) {
    ...
    if (ave_dit < 5) {
        ...
        PID_left = constrain(PID_left, 230, 255);
        PID_right = constrain(PID_right, 230, 255);
        Serial.print("\tp_term :");
        Serial.print(p_term);
    ...
    while(true){
      if (Serial.available()>0) {
        ...
        if (command == "arrival") {
          ...
          PID_lefts = constrain(PID_lefts, 210, 240);
          PID_rights = constrain(PID_rights, 210, 240);
          Serial.print("\tp_term :");
