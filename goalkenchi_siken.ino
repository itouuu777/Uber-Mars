...
double get_turn() {
  gp_data();
  get_oira();
  double currentYaw = oira_data[2]; 
  double goalDirection = gps_data[0];
  double relativeDirection = goalDirection - currentYaw;
...
void adjust() {
  while (true) {
    ...
    if (relativeDirection > -10 && relativeDirection < 10) {
      break;
    } else {
      // 右回りか左回りかを決定
      bool clockwise = relativeDirection > 0;
      if (clockwise) {
        spin_r(180, 180);
        delay(1000);
      }
